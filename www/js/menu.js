$(function() {
	...
	$.get('http://mis412.davidrichard.com/product_services/chico', function(data) {
		...
		for (var i = 0; i < data.categories.length; i++) {
			...
			for (var j = 0; j < data.categories[i].products.length; j++) {
			  $('#'+ data.categories[i].name).append(Mustache.render(product_template, data.categories[i].products[j]));
			};
